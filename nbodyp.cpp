...
#include <random>
#include <cmath>
#include "tooling/tooling/omp_loop.hpp"
#include <chrono>
#include <fstream>
...
void update_force_par(simulation &s, OmpLoop &omp)
{
...
void reset_force_par(simulation &s, OmpLoop &omp)
{
    omp.parfor(0, s.nbpart, 1, [&](size_t i)
               {
      s.fx[i] = 0.;
...
void updates_par(simulation &s, OmpLoop &omp, double dt)
{
    omp.parfor(0, s.nbpart, 1, [&](size_t i)
               {
      apply_force(s, i, dt);
...
int main(int argc, char *argv[])
{
    ...
    {
        std::cerr
            << "usage: " << argv[0] << " <input> <dt> <nbstep> <printevery> <nthreads>" << "\n"
            << "input can be:" << "\n"
    ...
    omp.setNbThread(std::atoi(argv[5]));
    ...
    for (size_t step = 0; step < nbstep; step++)
    {
        ...
        //parallelism contained in functions
        reset_force_par(s, omp);
        update_force_par(s, omp);
        updates_par(s, omp, dt);
    }
